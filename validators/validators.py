...
class BaseFieldValidator():
    ...
    def __init__(self, field_name = None, required = None, nullable = None):
        ...
        if required is None:
            raise ValidatorFieldError(field_name, "Missing required information if field '{validator_field_name}' is required.".format(validator_field_name=field_name))
        ...
        if nullable is None:
            raise ValidatorFieldError(field_name, "Missing required information if field '{validator_field_name}' can be null.".format(validator_field_name=field_name))
    ...
    def __init__(self, min_value = None, max_value = None, *args, **kwargs):
        ...
        if min_value is not None:
            if type(min_value) != int:
                raise ValidatorFieldError(self._name, "Invalid parameter 'min_value' for field {field_name} - received value type was {received_type}, expected was {expected_type}.".format(field_name=self._name, received_type=str(type(min_value)), expected_type=str(int) ))

        # checking 'max_value' parameter type
        if max_value is not None:
            if type(max_value) != int:
                raise ValidatorFieldError(self._name, "Invalid parameter 'max_value' for field {field_name} - received value type was {received_type}, expected was {expected_type}.".format(field_name=self._name, received_type=str(type(max_value)), expected_type=str(int) ))

        # checking if range 'min_value'-'max_value' is valid
        if min_value is not None and max_value is not None:
            if min_value > max_value:
                raise ValidatorFieldError(self._name, "Invalid 'min_value' and 'max_value' parameters configuration - 'min_value' is lesser than 'max_value'.")
...
class StringFieldValidator(BaseFieldValidator):
    ...
    def __init__(self, min_length = None, max_length = None, empty = None, *args, **kwargs):
        ...
        if min_length is not None:
            if type(min_length) == int:
                ...
                raise ValidatorFieldError(self._name, "'min_length' parameter type must be 'int'. Passed value type was {passed_value_type}".format(passed_value_type=type(min_length)))
        ...
        if max_length is not None:
            if type(max_length) == int:
                ...
                raise ValidatorFieldError(self._name, "'max_length' parameter type must be 'int'. Passed value type was {passed_value_type}".format(passed_value_type=type(max_length)))
        ...
        if empty is not None:
            if type(empty) == bool:
                ...
                raise ValidatorFieldError(self._name, "'empty' parameter type must be 'bool'. Passed value type was {passed_value_type}".format(passed_value_type=type(empty)))
        ...
        if min_length is not None and max_length is not None and empty is not None:
            if (min_length == 0 or max_length == 0) and not empty:
                raise ValidatorFieldError(self._name, "Neither 'min_length' nor 'max_length' can be 0, because field cannot be empty.")
    ...
    def __validate(self):
        ...
        if type(self._value) != str:
            raise ValidationError(self._name, "Type of provided value for field '{serializer_field_name}' is not str.".format(serializer_field_name=self._name))

        # check if field can be empty
        if self._empty is not None:
            if not self._empty:
                if len(self._value) == 0:
                    raise ValidationError(self._name, "Field '{serializer_field_name}' cannot be empty.".format(serializer_field_name=self._name))

        # compare grid value length with min_length
        if self._min_length is not None:
            if len(self._value) < self._min_length:
                raise ValidationError(self._name, "Provided value for field '{serializer_field_name}' is too short - it's length is equal to {value_length} but it should contain at least {min_length_value} characters.".format(serializer_field_name=self._name, value_length=len(self._value), min_length_value=self._min_length))

        # compare grid value length with max_length
        if self._max_length is not None:
            if len(self._value) > self._max_length:
                raise ValidationError(self._name, "Provided value for field '{serializer_field_name}' is too long - it's length is equal to {value_length} but it should contain max. {max_length_value} characters.".format(serializer_field_name=self._name, value_length=len(self._value), max_length_value=self._max_length))
...
class TicTacToeRequestValidator():
    ...
    def __init__(self, validator_data):
        ...
        for field_key in validator_fields:
            ...
            self.validated_data[field_key] = field_value # add field value to validated data dictionary (so it's not necessary to make it after validation => validation will raise any exception)
    ...
    def __validate(self):
        ...
        if len(grid_value) != grid_size_value * grid_size_value:
            raise ValidationError("grid", "Received grid length is irrelevant to declared grid size. Received grid length is {grid_length} but should be equal to {expected_grid_length}".format(grid_length=len(grid_value), expected_grid_length=(grid_size_value * grid_size_value)))

        # received grid value length validation
        if len(grid_value) != 9 and len(grid_value) != 16 and len(grid_value) != 25:
            raise ValidationError("grid", "Received grid state is invalid - it's length should be 9, 16 or 25. Length of received value was {received_grid_length}.".format(received_grid_length=len(grid_value)))

        # received grid value elements validation -> allowed values ['0', '1', '2']
        for i in range(0, len(grid_value)):
            if grid_value[i] not in ['0', '1', '2']:
                raise ValidationError("grid", "Invalid grid state identifier at index {invalid_index}.".format(invalid_index=i))
        ...
        if abs(x_fields - o_fields) > 1:
            raise ValidationError("grid", "Received grid is invalid - in tic-tac-toe game there no scenario to make this state happen.")
        ...
        if moving_player == 1 and x_fields > o_fields or moving_player == 2 and o_fields > x_fields:
            raise ValidationError("moving_player", "Requested player made more moves than opponent - cannot process this request.")
